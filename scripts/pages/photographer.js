...
function displaySortBy (medias) {
  ...
  const listOptions = [DropdownFirstOption, DropdownSecondOption, DropdownThirdOption]
...
function sortMedia (DropdownFirstOption, option, mediasList) {
  ...
  if (sortByDropdown.className === 'sortby-dropdown active') {
    const oldActiveText = DropdownFirstOption.textContent
    const newActiveText = option.textContent
...
function createGallery (mediasList) {
  ...
  mediasList.forEach(async (media) => {
    const { id, likes, photographerId, image, title, video } = media
    ...
    likeHeart.addEventListener('click', function (event) {
      ...
      const newLikes = parseInt(mediaLikes.textContent)
      if (newLikes != likes + 1) {
...
async function displayModalMedia (mediasList, id, type) {
  ...
  previousButton.addEventListener('click', function () {
    modalMedia.remove()
    const previousMedia = mediasList[index - 1]
  ...
  nextButton.addEventListener('click', function () {
    modalMedia.remove()
    const nextMedia = mediasList[index + 1]
  ...
  document.onkeydown = (e) => {
    const isModalNull = document.querySelector('.modal-media')
    if (isModalNull != null) {
      e = e || window.event
      if (e.keyCode === 37) {
        modalMedia.remove()
